...
@Component({
  selector: 'app-insert-need',
  templateUrl: './insert-need.component.html',
  styleUrls: ['./insert-need.component.scss'],
  animations: [
    trigger('leftright1', [
      state('right', style({
...
export class InsertNeedComponent implements OnInit {
  public left: boolean;
  ...
  public needForm = this.fb.group({
    needname: new FormControl('', [Validators.required]),
    prix: new FormControl('', [Validators.required]),
    needdescription: new FormControl('', [Validators.required]),
  ...
  moveRight(){
    if (localStorage.getItem("user")) {
      if (this.needForm.get('needname').value !== '' && this.needForm.get('needdescription').value !== '' && this.needForm.get('prix').value !== '') {
        this.left = !this.left;
  ...
  otherNext(){
    if ((this.needForm.get('distric').value !== '' && this.needForm.get('city').value !== '' && this.needForm.get('region').value !== '') || (this.needForm.get('distric').value !== '' && this.needForm.get('city').value !== '' && this.needForm.get('region').value !== '' && this.needForm.get('pays').value !== '')) {
  ...
  submitInsertService(event){
    ...
    formData.append("data",JSON.stringify(this.needForm.value));
    formData.append("userid",JSON.stringify(this.userid));
    setTimeout(()=>{
      this.handlingservice.handlingInsertNeed(this.url,formData).subscribe(
        res => {
          console.log(res);
