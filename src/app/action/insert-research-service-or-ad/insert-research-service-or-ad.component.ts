...
@Component({
  selector: 'app-insert-research-service-or-ad',
  templateUrl: './insert-research-service-or-ad.component.html',
  styleUrls: ['./insert-research-service-or-ad.component.scss'],
  animations: [
    trigger('leftright1', [
      state('right', style({
...
export class InsertResearchServiceOrAdComponent implements OnInit {
  ...
  public rechercheForm = this.fb.group({
    ...
    researchdescription: new FormControl('', [Validators.required]),
  ...
  moveRight(){
    if (localStorage.getItem("user")) {
      if (this.rechercheForm.get('researchname').value !== '' && this.rechercheForm.get('researchdescription').value !== '' && this.rechercheForm.get('prix').value !== '') {
        this.left = !this.left;
  ...
  otherNext(){
    if ((this.rechercheForm.get('distric').value !== '' && this.rechercheForm.get('city').value !== '' && this.rechercheForm.get('region').value !== '') || (this.rechercheForm.get('distric').value !== '' && this.rechercheForm.get('city').value !== '' && this.rechercheForm.get('region').value !== '' && this.rechercheForm.get('pays').value !== '')) {
  ...
  submitInsertService(event){
    ...
    formData.append("data",JSON.stringify(this.rechercheForm.value));
    formData.append("userid",JSON.stringify(this.userid));
    setTimeout(()=>{
      this.handlingservice.handlingInsertResearchService(this.url,formData).subscribe(
        res => {
          console.log(res);
        ...
        err => {
          console.log(err);
          this.isLoader = false;
          alert("Probléme de connexion, Veillé reessayer ultèrierement")
        }
      )
    },3000);
